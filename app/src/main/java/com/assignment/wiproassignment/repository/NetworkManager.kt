...
import com.assignment.wiproassignment.repository.ResponseCodes
...
class NetworkManager() {
    ...
    fun <V> createApiRequest(observables: Observable<V>, callBack: ServiceListener<V>) {
        // progressShow(context, messageText)
    ...
    private fun setUpErrors(t: Throwable): ErrorModel {
        ...
        try {
            ...
            if (t is SocketTimeoutException) {
                errorModel.error_code = ResponseCodes.Companion.INTERNET_NOT_AVAILABLE
                errorModel.error_message = ResponseCodes.Companion.logErrorMessage(errorModel.error_code)
            } else if (t is TimeoutException) {
                errorModel.error_code = ResponseCodes.Companion.URL_CONNECTION_ERROR
                errorModel.error_message = ResponseCodes.Companion.logErrorMessage(errorModel.error_code)
            } else if (t is ClassCastException) {
                errorModel.error_code = ResponseCodes.Companion.MODEL_TYPE_CAST_EXCEPTION
                errorModel.error_message = ResponseCodes.Companion.logErrorMessage(errorModel.error_code)
            ...
            ex.printStackTrace()
            errorModel.error_code = ResponseCodes.Companion.UNKNOWN_ERROR
            errorModel.error_message = ResponseCodes.Companion.logErrorMessage(errorModel.error_code)
        } finally {
            progressHide()
        }
